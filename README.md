...
The only portions of the helm chart that are production ready are the parts that configure and manage Temporal Server itself—not Cassandra, or Elasticsearch.
...
By default, Temporal Helm Chart configures Temporal to run with a three node Cassandra cluster (for persistence) and Elasticsearch (for "visibility" features). By default, Temporal Helm Chart installs all dependencies, out of the box.
